...
op_palette <- c("#FF6A10", "#6E6E6E", "#C37E00", "#7F8700", "#2B8700", "#00802F", "#00765D", "#E0E0E0", "#FFA26B", "#CC9E4D", "#969742", "#628D47", "#297E53", "#006D5E", "#A5A5A5", "#A23C00", "#8F5D00", "#757600", "#548A2B", "#1A9A5E", "#00A981")

# Take a look at the colors
# pie(rep(1, length(op_palette)), col = op_palette, labels = op_palette)

# Declare font for plotting
windowsFonts(Chevin = windowsFont("OP Chevin Pro Light"))
...
analyzer <- function(dat, parameter, parameter_name, suffix = "", min = 0) {
  ...
  if (min == 1) {
    ...
    plot_dates <- ggplot(data = dat, aes(x = date, group = 1)) +
      ...
      scale_color_manual(name = parameter_name, values = op_palette, guide = guide_legend(fill = NULL, colour = op_palette)) +
    ...
    plot_dates <- ggplot(data = dat, aes(x = date, group = 1)) +
      ...
      scale_color_manual(name = parameter_name, values = op_palette, guide = guide_legend(fill = NULL, colour = op_palette)) +
      scale_fill_manual(values = op_palette)
    # Save Plot
...
correlations <- function(dataset, output_name, method){
  ...
  corPlot <- ggplot(data = melted_cormat, aes(Var2, Var1, fill = value))+
    ...
    scale_fill_gradient2(low = "darkblue", high = op_palette[1], mid = "grey90", 
...
correlation_check <- function(data, parameter_1, parameter_2, parameter_1_name, parameter_2_name, factor = 1) {
  ...
  plot_correlations <- ggplot(data = data, aes(x = date, group = 1)) +
    ...
    scale_color_manual(name = "Series", values = op_palette, guide = guide_legend(fill = NULL, colour = op_palette))
